document.addEventListener("DOMContentLoaded", async () => {
  ...
  document
    ...
    .addEventListener("click", buscarEmpresas);
  ...
  document
    .getElementById("visitaForm")
    .addEventListener("submit", handleSubmit);

  // Event listener para mudança no tipo de visita
  document
    .getElementById("type")
    .addEventListener("change", handleTipoVisitaChange);
...
const handleTipoVisitaChange = (e) => {
  ...
  const prospeccaoInfo = document.getElementById("prospeccaoInfo");
  const empresaBuscaContainer = document.getElementById(
    "empresaBuscaContainer"
  );
  const empresaManualContainer = document.getElementById(
    "empresaManualContainer"
  );
  const empresaSearch = document.getElementById("empresaSearch");
  const empresaManual = document.getElementById("empresaManual");
  const companyIdField = document.getElementById("company_id");
  const cityIdField = document.getElementById("city_id");
  const isProspeccaoField = document.getElementById("is_prospeccao");

  // Resetar visibilidade
  trabalhoInternoInfo.style.display = "none";
  prospeccaoInfo.style.display = "none";
  ...
  if (tipoVisita === "TRABALHO INTERNO") {
    ...
    if (!objetivoField.value.trim()) {
      objetivoField.placeholder =
        "Ex: REUNIÃO INTERNA, TREINAMENTO, PLANEJAMENTO ESTRATÉGICO, ANÁLISE DE PROCESSOS...";
    }
  } else if (tipoVisita === "PROSPECÇÃO DE CLIENTE") {
    prospeccaoInfo.style.display = "block";
    empresaBuscaContainer.style.display = "none";
    empresaManualContainer.style.display = "block";

    // Tornar campo manual obrigatório e busca opcional
    empresaManual.required = true;
    empresaSearch.required = false;
    companyIdField.required = false;
    cityIdField.required = false;
    isProspeccaoField.value = "1";
    ...
    // Sugerir objetivo para prospecção
    const objetivoField = document.getElementById("objetivo");
    if (!objetivoField.value.trim()) {
      objetivoField.placeholder =
        "Ex: APRESENTAÇÃO COMERCIAL, IDENTIFICAÇÃO DE NECESSIDADES, PROPOSTA DE PARCERIA...";
    }
  } else {
    empresaBuscaContainer.style.display = "block";
    empresaManualContainer.style.display = "none";

    // Tornar busca obrigatória
    empresaSearch.required = true;
    empresaManual.required = false;
    companyIdField.required = true;
    cityIdField.required = true;
    isProspeccaoField.value = "0";
...
const buscarEmpresas = async () => {
  ...
  try {
    ...
    const response = await window.apiCall(
      `/empresas.php?search=${encodeURIComponent(searchTerm)}&limit=20`
    );
    ...
    if (response.success && response.data && response.data.length > 0) {
      ...
      resultsContainer.innerHTML =
        '<div class="search-result-item">NENHUMA EMPRESA ENCONTRADA</div>';
...
const mostrarResultados = (empresas) => {
  ...
  resultsContainer.innerHTML = empresas
    .map(
      (empresa) => `
    <div class="search-result-item" onclick="selecionarEmpresa(${JSON.stringify(
      empresa
    ).replace(/"/g, "&quot;")})">
...
const buscarCityIdPorNome = async (cidadeNome) => {
  try {
    ...
    const estadosRestantes = estadosIds.filter(
      (id) => !estadosPrioritarios.includes(id)
    );
    ...
    for (const estadoId of ordemBusca) {
      try {
        ...
        const response = await window.apiCall(
          `/estados-cidades.php?estado_id=${estadoId}`
        );
...
const buscarCityIdValido = async () => {
  try {
    ...
    if (response.success && response.data && response.data.length > 0) {
      ...
      console.log(
        "✅ Primeira cidade de MG encontrada:",
        primeiraCidade.nome,
        "ID:",
        primeiraCidade.id
      );
...
const selecionarEmpresa = async (empresa) => {
  ...
  // 2. Se não tem city_id mas tem cidade_nome, buscar pelo nome
    ...
    console.log(
      "🔍 Empresa sem city_id, buscando pelo nome da cidade:",
      empresa.cidade_nome
    );
  ...
  if (!cityId) {
    console.log(
      "⚠️ Não foi possível encontrar city_id específico, buscando cidade padrão..."
    );
  ...
  document.getElementById("empresaCidade").textContent =
    empresa.cidade_nome || "CIDADE PADRÃO";
  document.getElementById("empresaEndereco").textContent =
    empresa.address || "-";
  document.getElementById("empresaTelefone").textContent = empresa.phone || "-";
  document.getElementById("empresaWhatsapp").textContent =
    empresa.whatsapp || "-";
  document.getElementById("empresaResponsavel").textContent =
    empresa.responsible || "-";
...
const handleSubmit = async (e) => {
  ...
  const tipoVisita = document.getElementById("type").value;
  const isProspeccao = tipoVisita === "PROSPECÇÃO DE CLIENTE";

  if (isProspeccao) {
    const empresaManual = document.getElementById("empresaManual").value.trim();
    if (!empresaManual) {
      window.showError("DIGITE O NOME DA EMPRESA QUE ESTÁ PROSPECTANDO");
      return;
    }

    // Para prospecção, usar city_id padrão se não tiver
    let cityId = document.getElementById("city_id").value;
    if (!cityId || cityId === "" || cityId === "0") {
      console.log("⚠️ Prospecção sem city_id específico, buscando padrão...");
      cityId = await buscarCityIdValido();
      document.getElementById("city_id").value = cityId;
    }
  } else {
    // Para visitas normais, validar empresa cadastrada
  ...
  try {
    ...
    if (response.success) {
      ...
      if (isProspeccao) {
        successMessage = "PROSPECÇÃO DE CLIENTE AGENDADA COM SUCESSO!";
      }
