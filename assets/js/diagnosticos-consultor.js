// ========================================
// DIAGNÓSTICOS POR CONSULTOR - SISTEMA INDEPENDENTE
// ========================================
...
document.addEventListener("DOMContentLoaded", async () => {
  console.log("📋 Diagnósticos por Consultor carregado")
  ...
  document.getElementById("clearFilters").addEventListener("click", () => {
    setCurrentMonthDates()
    document.getElementById("consultorFilter").value = ""
    currentPage = 1
    loadDiagnosticos()
  })
  ...
  document.getElementById("consultorFilter").addEventListener("change", () => {
    currentPage = 1
    loadDiagnosticos()
  })
  ...
  console.log("✅ Diagnósticos por Consultor configurado com sucesso")
})
...
const loadConsultores = async () => {
  try {
    console.log("🔄 Carregando lista de consultores...")

    const response = await DiagnosticosAPI.call("/usuarios.php")

    if (response && response.success && response.data) {
      consultoresList = response.data
      populateConsultoresSelect()
      console.log(`✅ ${consultoresList.length} consultores carregados`)
    }
  } catch (error) {
    console.error("❌ Erro ao carregar consultores:", error)
    DiagnosticosNotify.error("Erro ao carregar lista de consultores")
  }
}

const populateConsultoresSelect = () => {
  ...
  consultoresList.forEach((consultor) => {
...
const getFilters = () => {
  ...
  const consultorId = document.getElementById("consultorFilter").value

  if (dataInicio) filters.data_inicio = dataInicio
  if (dataFim) filters.data_fim = dataFim
  if (consultorId) filters.consultor_id = consultorId
