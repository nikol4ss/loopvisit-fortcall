...
function navigateToPrevWeek() {
  ...
  console.log(
    "Navegando para semana anterior:",
    currentWeekStart,
    "até",
    currentWeekEnd
  );
...
function updateWeekDisplay() {
  ...
  if (weekPeriodElement)
    weekPeriodElement.textContent = `${startFormatted} até ${endFormatted}`;
}
...
function loadDashboardData() {
  ...
  if (tableBody) {
    tableBody.innerHTML =
      '<tr><td colspan="8" class="text-center">Carregando visitas da semana...</td></tr>';
  }

  // Preparar parâmetros com formato correto (sem fuso horário)
  ...
  console.log(
    "Data fim:",
    dataFim,
    "(" + currentWeekEnd.toLocaleDateString("pt-BR") + ")"
  );
...
function renderVisitasTable(visitas) {
  ...
  visitas.forEach((visita) => {
    ...
    if (visita.is_retroativa == 1) {
      empresaInfo += `<br><span class="badge-retroativa">RETROATIVA</span>`;
    }

    // Buscar data do check-in em múltiplos campos
    let dataCheckin = "-";
    ...
    row.innerHTML = `
      ...
      <td><span class="status-badge status-${(
        visita.status_calculado || ""
      ).toLowerCase()}">${visita.status_calculado}</span></td>
...
// Formatar data para API sem problemas de fuso horário
...
function getActionButtons(visita) {
  ...
  switch (visita.status_calculado) {
    ...
    case "ATRASADA":
      buttons.push(
        `<a href="visitas/checkin.html?id=${visita.id}" class="btn-action btn-checkin">CHECK-IN</a>`
      );
      break;

    case "REALIZADA":
      buttons.push(
        `<a href="visitas/checkin.html?id=${visita.id}" class="btn-action btn-view">VER CHECK-IN</a>`
      );
      break;
