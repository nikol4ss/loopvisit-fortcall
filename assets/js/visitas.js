...
document.addEventListener("DOMContentLoaded", async () => {
  ...
  if (closeCancelBtn) {
    closeCancelBtn.addEventListener("click", fecharModalCancelamento)
  }
  ...
  try {
    ...
    response.data.forEach((visita) => {
      ...
      if (visita.status === "CANCELADA" && cancelReasons[visita.id]) {
        statusContent += `<br><small class="cancel-motivo">${cancelReasons[visita.id]}</small>`
      }
      ...
      row.innerHTML = `
        <td>${visita.is_prospeccao == 1 ? visita.empresa_livre : visita.empresa_nome}</td>
        <td>${dataFormatada}</td>
        <td>${visita.type}</td>
...
const cancelar = (visitaId) => {
  console.log("[v0] Abrindo modal de cancelamento para visita:", visitaId)
  visitaParaCancelar = visitaId
  const modal = document.getElementById("cancelModal")
  if (modal) {
    modal.style.display = "flex" // Usando flex ao invés de block para centralizar
    document.getElementById("cancelMotivo").value = ""
    document.getElementById("cancelMotivo").focus()
  } else {
    console.error("[v0] Modal de cancelamento não encontrado")
  }
}
...
const confirmarCancelamento = async () => {
  ...
  console.log("[v0] Confirmando cancelamento da visita:", visitaParaCancelar, "Motivo:", motivo)
  ...
  try {
    ...
    console.error("[v0] Erro ao cancelar visita:", error)
    window.showError("ERRO AO CANCELAR VISITA")
  }
}

const fecharModalCancelamento = () => {
  console.log("[v0] Fechando modal de cancelamento")
  const modal = document.getElementById("cancelModal")
  if (modal) {
    modal.style.display = "none"
  }
  document.getElementById("cancelMotivo").value = ""
  visitaParaCancelar = null
}
